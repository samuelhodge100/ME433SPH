...
# IIR
for i in range(0,len(filt_data2)):
    IIR_data2.append(A*filt_data2[i] + B*data2[i+num_avg-1])
    IIR_t.append(data1[i+num_avg-1])

Fs = 1/data1[1]# sample rate
Ts = data1[1]; # sampling interval
ts = np.arange(0,t[-1],Ts) # time vector     
y = IIR_data2 # the data to make the fft from
...
fig.suptitle("IIR for "+str(path)+" ("+str(num_avg)+" data points and A:"+str(A)+", B:"+ str(B) +")")
plt.show()
